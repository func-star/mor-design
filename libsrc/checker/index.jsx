...
export default class Checker extends Component {
	...
	getValue () {
		const { multiple, type, emptyValue } = this.props
		//选择为空时，可设置默认值
	...
	setValue (val, childFresh = true) {
		...
		if (childFresh) {
			Tool.queryReactDomAll(this.refs.wrap, '.mona-checker-item').forEach((v) => {
				v.checkSelect()
			})
		}
	}
	...
	render () {
		const { className, children, formIgnore, node, item, emptyValue, ...props } = this.props
		if (node) {
			let Node = node
			return (
				<Node className={classNames('mona-checker', { 'mona-form-control': !formIgnore }, className)} ref="wrap">
					{children}
				</Node>
			)
		} else {
			return (
				<div className={classNames('mona-checker', { 'mona-form-control': !formIgnore }, className)} ref="wrap">
					{children}
				</div>
			)
		}
