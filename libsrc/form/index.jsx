...
export default class Form extends Component {
	...
	getChildren (a) {
		let formControls
		let dom = findDOMNode(this.refs.form)
		if (this.props.filterNestChild) {
			let nestChild = Tool.toArray(dom.querySelectorAll(':scope .mona-form-control .mona-form-control'))
			if (a) {
			}
			formControls = Tool.queryReactDomAll(dom, ':scope .mona-form-control', (doms) => {
				return Tool.toArray(doms).filter((v) => {
					return nestChild.indexOf(v) < 0
				})
			})
		} else {
			formControls = Tool.queryReactDomAll(dom, ':scope .mona-form-control')
		}
		return formControls
	}
	...
	render () {
		...
		if (node) {
			let Node = node
			return (
				<Node className={classNames('mona-form', className)} {...props} onSubmit={this.onSubmit.bind(this)} ref="form">
					{children}
				</Node>
			)
		}
		return (
			<form className={classNames('mona-form', className)} {...props} onSubmit={this.onSubmit.bind(this)} ref="form">
				{children}
			</form>
		)
	}
